    ...
    try:
        if os.path.exists(RL_FILE):
            with open(RL_FILE, "r") as f:
                data = json.load(f)
                # Ensure proper types
                return {k: {ak: float(av) for ak, av in v.items()} for k, v in data.items()}
    except Exception:
        pass
    return {}


def rl_save(store: Dict[str, Dict[str, float]]) -> None:
    try:
...
def ttt_generate_candidates(board: List[List[str]]) -> List[Tuple[int, int]]:
    # Consider empty cells adjacent within distance 1 to any occupied cell; fallback to all if board empty
    ...
    if not nbrs:
        return empties
    random.shuffle(nbrs)
    return nbrs[: min(18, len(nbrs))]


def ttt_minimax(board: List[List[str]], depth: int, alpha: int, beta: int, maximizing: bool) -> Tuple[int, Optional[Tuple[int, int]]]:
    ...
    if maximizing:
        value = -10**9
        # Move ordering: try immediate wins first
        for r, c in candidates:
        ...
        for r, c in candidates:
            board[r][c] = AI
            eval_score, _ = ttt_minimax(board, depth - 1, alpha, beta, False)
            board[r][c] = EmptyCell
            if eval_score > value:
                value = eval_score
                best_move = (r, c)
        ...
        value = 10**9
        # Opponent immediate win avoidance in lookahead
        for r, c in candidates:
        ...
        for r, c in candidates:
            board[r][c] = Human
            eval_score, _ = ttt_minimax(board, depth - 1, alpha, beta, True)
            board[r][c] = EmptyCell
            if eval_score < value:
                value = eval_score
                best_move = (r, c)
...
def ttt_ai_move(board: List[List[str]]) -> Tuple[int, int]:
    ...
    # Epsilon exploration
    explore = random.random() < RL_EPSILON
    ...
    if chosen is None:
        ...
        depth = 3 if len(empties) <= 18 else 2
        _, chosen = ttt_minimax(board, depth=depth, alpha=-10**9, beta=10**9, maximizing=True)
